...
internals.DefaultContentLoader = function(logger, appSettings) {
    ...
    */
    ...
    this.load = function(request, callback)
    {
        ...
        {
            var element = self.itemDescriptions_[theme.items[idx]];
            var module = require(element.module + '/' + element.renderer );
            var Renderer = module[element.renderer];
            var renderer = new Renderer(self.logger_, self.appSettings_, element.config);
            renderer.render(request, function(error, result){
                if (!error) {
                    renderedItems[element.name] = {result: result};
                } else {
                    renderedItems[element.name] = {error: error};
                }
